...
def xt_var(φ: float, npts: int) -> numpy.ndarray[float]:
    """
    ...
    return (npts + sum) / (1.0 - φ**2)

def yt_var(φ: float, β: float, npts: int) -> numpy.ndarray[float]:
    """
    ...
    return xt_var(φ, npts) * β**2

def cov(φ: float, β: float, npts: int) -> numpy.ndarray[float]:
    """
