...
def ecm_beta(data: numpy.ndarray[float], results: OLSSingleVarResult, x: numpy.ndarray[float]=None, **kwargs):
    """"
    Plot the results of an FBM variance aggregation analysis used to estimate the Hurst parameter.
    ...
    if title is None:
        kwargs["title"] = f"ECM $\beta$ Estimation"

    transform = results.transform
    const = results.const.est
    beta = transform.param.est
    func = lambda x: const + beta * x
