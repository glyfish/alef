...
class TestMAqClosedForms:
    ...
    def test_maq_acf_independent_of_sigma(self):
        # ACF is a correlation: σ² cancels between maq_cov and maq_sigma².
        assert_allclose(model.maq_acf([0.6], σ=1.0), model.maq_acf([0.6], σ=3.5), rtol=1e-12)
...
class TestARMA:
    def test_arma11_closed_form_acf_and_variance(self):
        ...
        acf = sample_acf(x, 3)
        # SE(r_k) ≈ sqrt((1-ρ1²)/n) ≈ 0.01 → 0.05 is ~5σ
        assert_allclose(acf[1:], [ρ1, φ * ρ1, φ**2 * ρ1], atol=0.05)
...
class TestYuleWalker:
    def test_returns_order_coefficients(self):
        coeffs = model.yw(model.ar1(0.5, 500), 3)
...
class TestFacadeClosedForms:
    ...
    def test_compute_maq_acf_sigma_default_matches_model(self):
        _, acf = facade.compute_maq_acf(θ=[0.4, 0.2], nlags=3)
        assert_allclose(acf, model.maq_acf([0.4, 0.2], 1.0, 3), rtol=1e-12)
...
class TestFacadeSources:
    ...
    def test_contract_and_agreement_with_model_layer(self, source, kwargs, model_call):
        reseed()
        t, x = source(**kwargs)
        reseed()
        expected = model_call()
...
class TestFacadeEstimates:
    ...
    def test_estimate_contract(self, estimate, est_type, label_symbol):
        ...
        for i, p in enumerate(est.params):
            ...
            assert label_symbol in p.est_label
